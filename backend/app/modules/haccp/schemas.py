...
class HACCPPlanReviewBase(BaseModel):
    plan_id: UUID
    review_date: date
    review_type: str = "PERIODIC"  # PERIODIC | TRIGGERED_BY_CHANGE | POST_INCIDENT
    triggered_by_change_id: Optional[UUID] = None
    reviewed_by_name: str
    scope_of_review: str
    findings: str
    changes_required: bool = False
    plan_version_before: str = "1.0"
    plan_version_after: str = "1.0"
    approval_status: str = "APPROVED"  # APPROVED | REVISION_REQUIRED
    approved_by_name: Optional[str] = "Đội trưởng Đội ATTP"

class HACCPPlanReviewCreate(HACCPPlanReviewBase):
    review_code: Optional[str] = None

class HACCPPlanReviewResponse(HACCPPlanReviewBase):
    model_config = ConfigDict(from_attributes=True)
...
# ==================== 10. SYNC FLOW STEPS FROM WORKFLOW BUILDER ====================
class SyncFlowStepItem(BaseModel):
    step_id: Optional[Union[UUID, str]] = None
    step_number: Optional[int] = 1
    step_name: str = Field(..., max_length=255, description="Tên công đoạn sản xuất")
    product_line: Optional[str] = "Chế biến Thủy hải sản"
    description: Optional[str] = None
    is_ccp_or_oprp: Optional[bool] = False

    model_config = ConfigDict(extra="ignore")

class SyncFlowStepsRequest(BaseModel):
    steps: List[SyncFlowStepItem]
