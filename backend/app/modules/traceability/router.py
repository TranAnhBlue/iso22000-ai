...
from app.modules.purchasing.models import MaterialLot, Supplier, IQCInspection
from app.modules.haccp.models import CCPMonitoringLog, CCPDefinition
...
from app.modules.inventory.router import enrich_inventory_fefo

router = APIRouter()

def format_usage_res(u: Any) -> BatchMaterialUsageResponse:
    return BatchMaterialUsageResponse(
        usage_id=u.usage_id,
        material_lot_id=u.material_lot_id,
        material_name=str(u.material_name),
        lot_number=str(u.lot_number),
        quantity_used=float(u.quantity_used or 0.0),
        unit=str(u.unit),
        recorded_at=u.recorded_at,
    )

def format_batch_res(b: Any, usages_res: List[BatchMaterialUsageResponse], ccp_logs_count: int) -> ProductionBatchResponse:
    return ProductionBatchResponse(
        batch_id=b.batch_id,
        batch_number=str(b.batch_number),
        product_name=str(b.product_name),
        product_code=str(b.product_code) if b.product_code else None,
        production_line=str(b.production_line) if b.production_line else None,
        shift=str(b.shift) if b.shift else None,
        planned_quantity=float(b.planned_quantity or 0.0),
        actual_quantity=float(b.actual_quantity or 0.0),
        unit=str(b.unit),
        start_time=b.start_time,
        end_time=b.end_time,
        status=str(b.status),
        qc_inspector=str(b.qc_inspector) if b.qc_inspector else None,
        notes=str(b.notes) if b.notes else None,
        material_usages=usages_res,
        ccp_logs_count=ccp_logs_count,
        created_at=b.created_at,
    )

def format_supplier_node(u: Any, mat_lot: Any, supplier: Any, iqc: Any) -> TraceabilitySupplierNode:
    return TraceabilitySupplierNode(
        supplier_code=str(supplier.supplier_code) if supplier and supplier.supplier_code else "NCC-EXT",
        supplier_name=str(supplier.supplier_name) if supplier and supplier.supplier_name else "Nhà cung ứng chỉ định",
        rating_score=float(supplier.rating_score) if supplier and supplier.rating_score is not None else 98.0,
        certifications=supplier.certifications if supplier and supplier.certifications else ["ISO 22000:2018", "HACCP"],
        lot_number=str(u.lot_number),
        material_name=str(u.material_name),
        received_date=mat_lot.received_date if mat_lot else None,
        iqc_status=str(iqc.status if iqc and iqc.status else (mat_lot.status if mat_lot and mat_lot.status else "APPROVED")),
        iqc_inspector=str(iqc.inspector.full_name) if iqc and iqc.inspector and iqc.inspector.full_name else "QC Tiếp nhận",
        coa_file_url=str(mat_lot.coa_file_url) if mat_lot and mat_lot.coa_file_url else None,
    )

def format_ccp_node(log: Any) -> TraceabilityCCPNode:
    ccp_def = log.ccp
    return TraceabilityCCPNode(
        ccp_code=str(ccp_def.ccp_code) if ccp_def and ccp_def.ccp_code else "CCP",
        ccp_name=str(ccp_def.name) if ccp_def and ccp_def.name else "Điểm kiểm soát",
        process_step=str(ccp_def.process_step.step_name) if ccp_def and ccp_def.process_step and ccp_def.process_step.step_name else "Công đoạn chế biến",
        measured_value=float(log.measured_value or 0.0),
        unit=str(log.unit or "°C"),
        critical_limit=ccp_def.critical_limit if ccp_def and ccp_def.critical_limit else {},
        is_critical_limit_exceeded=bool(log.is_critical_limit_exceeded),
        status=str(log.status or "NORMAL"),
        test_time=log.test_time,
        checked_by_name=str(log.inspector.full_name) if log.inspector and log.inspector.full_name else "QC Vận hành",
    )

def format_sample_node(s: Any) -> TraceabilitySampleNode:
    return TraceabilitySampleNode(
        sample_code=str(s.sample_code),
        storage_cabinet=str(s.storage_cabinet),
        sample_date=s.sample_date,
        expiry_date=s.expiry_date,
        test_result=str(s.test_result),
        status=str(s.status),
    )

def format_dispatch_node(d: Any, default_status: Optional[str] = None) -> TraceabilityCustomerNode:
    status_val = default_status if default_status else str(d.status)
    return TraceabilityCustomerNode(
        dispatch_code=str(d.dispatch_code),
        order_number=str(d.order_number),
        customer_name=str(d.customer_name),
        customer_phone=str(d.customer_phone) if d.customer_phone else None,
        destination_address=str(d.destination_address) if d.destination_address else None,
        quantity_dispatched=float(d.quantity_dispatched or 0.0),
        unit=str(d.unit),
        vehicle_number=str(d.vehicle_number) if d.vehicle_number else None,
...
def backward_traceability(
    ...
    usages_res = [format_usage_res(u) for u in usages]

    ccp_logs_count = int(db.query(func.count(CCPMonitoringLog.log_id)).filter(
        CCPMonitoringLog.batch_number == batch.batch_number
    ).scalar() or 0)

    batch_response = format_batch_res(batch, usages_res, ccp_logs_count)
    ...
    for u in usages:
        ...
        supplier_nodes.append(format_supplier_node(u, mat_lot, supplier, iqc))
    ...
    ccp_nodes: List[TraceabilityCCPNode] = [format_ccp_node(log) for log in ccp_logs]
    ...
    sample_nodes = [format_sample_node(s) for s in samples]
    ...
    dispatch_nodes = [format_dispatch_node(d) for d in dispatches]
    ...
    total_stock_qty = sum(float(getattr(s, "quantity", 0.0) or 0.0) for s in stocks)
    is_safe = all_ccp_ok and all_iqc_ok and (str(batch.status) == "COMPLETED")
...
def forward_traceability_mock_recall(
    ...
    affected_customers = [
        format_dispatch_node(d, default_status="RECALLED" if str(d.status) == "RECALLED" else "DELIVERED")
        for d in dispatches
    ]
