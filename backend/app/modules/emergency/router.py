...
def update_emergency_contact(contact_id: UUID, contact_in: EmergencyContactUpdate, db: Session = Depends(get_db)):
    ...
    if not contact:
        raise HTTPException(status_code=404, detail="Không tìm thấy liên hệ khẩn cấp.")
...
def delete_emergency_contact(contact_id: UUID, db: Session = Depends(get_db)):
    ...
    db.delete(contact)
    db.commit()
    return {"message": "Đã xóa liên hệ khẩn cấp."}
...
def get_emergency_procedures(
    ...
    if risk_level and risk_level != "ALL":
        query = query.filter(EmergencyProcedure.risk_level == risk_level)
    ...
    procedures = query.order_by(asc(EmergencyProcedure.procedure_code)).all()
    return procedures


@router.get("/procedures/{procedure_id}", response_model=EmergencyProcedureResponse)
def get_emergency_procedure_by_id(procedure_id: UUID, db: Session = Depends(get_db)):
...
def create_emergency_procedure(proc_in: EmergencyProcedureCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"Mã quy trình '{code_str}' đã tồn tại.")

    # Calculate risk_score and risk_level
    score = proc_in.likelihood * proc_in.severity
    if score <= 4:
        level = "LOW"
    elif score <= 9:
        level = "MEDIUM"
    ...
    proc = EmergencyProcedure(**proc_in.model_dump())
    proc.risk_score = score
    proc.risk_level = level
...
def update_emergency_procedure(procedure_id: UUID, proc_in: EmergencyProcedureUpdate, db: Session = Depends(get_db)):
    ...
    if proc.risk_score <= 4:
        proc.risk_level = "LOW"
    elif proc.risk_score <= 9:
        proc.risk_level = "MEDIUM"
    elif proc.risk_score <= 15:
