...
- [x] **Phase 10a: Hoàn thiện FSMS ISO 22000 AI Hub theo Tài liệu Gốc & Chuẩn ISO 22000:2018:**
  - **1. Chuẩn hóa Biên bản Tiêu hủy BM02-HỦY HÀNG (`/inventory`):** Khớp chính xác `BM02-BIÊN BẢN XÁC NHẬN HỦY HÀNG.xlsx` với bảng 3 đại diện tham gia ở đầu biên bản và 3 khối chữ ký ở cuối (Đơn vị thực hiện hủy hàng, P.QLCL, Đơn vị đề xuất hủy hàng).
  - **2. Hoạch định sự thay đổi hệ thống FSMS (Clause 6.3 - `/change-management`):** Bảng `change_requests`, API CRUD `/change-management/requests`, phân quyền RBAC `change_management` trên 8 vai trò, giao diện `/change-management`, đánh giá tác động 3 trụ cột (HACCP, PRP, Khẩn cấp), và in biên bản thay đổi A4 **BM-CHANGE-01**.
  - **3. Sổ nhật ký trao đổi thông tin ATTP nội bộ & bên ngoài (Clause 7.4 - `/organization`):** Bảng `communications_log`, API CRUD `/organization/communications`, giao diện nhật ký trao đổi thông tin đa kênh (nội bộ, cơ quan chức năng, khách hàng, nhà cung ứng), phân loại khẩn cấp, theo dõi hạn phản hồi và in sổ theo dõi **BM-COMM-01**.
  - **4. Đội An toàn thực phẩm chính thức (Clause 5.3 & QĐ 02 - `/organization`):** Bảng `food_safety_team_members`, bám sát tuyệt đối văn bản gốc `02 QD Thanh lap doi ATTP.doc` với đúng 3 chức danh: **Đội trưởng**, **Thư ký**, **Đội viên**, phân công nhiệm vụ, người thay thế khi vắng mặt, và in quyết định bổ nhiệm chuẩn **BM-FST-01**.
  - **5. Thẩm tra định kỳ kế hoạch HACCP tổng thể (Clause 8.6 & 8.8 - `/haccp`):** Bảng `haccp_plan_reviews`, API CRUD `/haccp/reviews`, liên kết tự động với Yêu cầu thay đổi (Clause 6.3) khi thẩm tra sau thay đổi, đánh giá 3 trụ cột (Mối nguy còn hiệu lực, Hồ sơ CCP/OPRP đầy đủ, Hiệu lực CAPA), kết luận hiệu lực và in biên bản thẩm tra **BM-HACCP-REV-01**.
